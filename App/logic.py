...
def load_books_tags(catalog):
    ...
    lbookstags = app.get("book_tags")
    book_tags_file = os.path.join(data_dir, filename)
    app["books"] = set.load_set(lbookstags, book_tags_file)
    if empty_books(app):
        return None
    else:
...
def get_best_books(catalog, number):
    ...
    top_ratings=sorted(catalog["book_tags"]["average_ratings"],reverse=true)
    return top_ratings[:number]
